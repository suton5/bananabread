    ...
    biases( biases_ )
{
	...
	h = std::get<ActivationFnType>( architecture["activation_fn"] );
}
...
Eigen::MatrixXd FeedForward::forward_pass( Eigen::VectorXd x )
{
	...
	int index_b = width - 1;
	Eigen::MatrixXd W
	    = weights( Eigen::seq( 0, index_W ) ).reshaped( width, D_in );
	...
	// output layer
	W = weights( Eigen::seq( index_W + 1, index_W + width * D_out ) )
	        .reshaped( D_out, width );
